...
import type { Order, OrdersByDate, OrderItem, AddOnCount } from '../types'
...
export const getNextOrderId = (): number => {
  ...
  return todayOrders.length > 0
    ? Math.max(...todayOrders.map((o: Order) => o.id)) + 1
    : 1
}
...
export const exportOrdersAsCSV = (date?: string): void => {
  ...
  const rows = orders.map((order: Order) => {
    const prepTime = order.preparationTime
      ? `${Math.floor(order.preparationTime / 60)}:${(order.preparationTime % 60)
          .toString()
          .padStart(2, '0')}`
      : 'N/A'
      ...
      order.items
        .map((item: OrderItem) => {
          ...
          if (item.addOns.length > 0) {
            itemStr += ` (+ ${item.addOns
              .map((a: AddOnCount) => `${a.addOn.name} x${a.count}`)
              .join(', ')})`
          }
          return itemStr
        })
